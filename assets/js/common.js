...
$.extend(true, $.fn.dataTable.defaults, {
    ...
    paging      : true,
    processing  : true,
    serverSide  : true, // Only keep true if you're using AJAX source
    searching   : false,
    columnDefs  : [
...
function sweetalert(msg='',url='',callback ='',title='') {
  ...
  switch (msg) {
    ...
    case "form_alert_group":
      Swal.fire({
        icon: 'info',
        title: 'Fill Out Group Fields',
        // imageUrl:'img/emoji/form_fill.webp',
        showConfirmButton: true,
        timer: 2000,
        timerProgressBar: true
...
// Override add_doc_type to close modal and update select without reload
function add_doc_type(name) {
  if (name) {
    name = name.trim();
  }
  var data = {
  ...
  var ajaxurl = sessionStorage.getItem('folder_crud_link');
  $.post(ajaxurl, data, function(response) {
    ...
    if (typeof response === "string") {
      try {
        ...
        res = { status: false, msg: "Invalid response", details: response };
      }
    }
    if (res && (res.status === 1 || res.status === true || res.msg === "success")) {
      // Close modal
      var modal = bootstrap.Modal.getInstance(document.getElementById('addDocTypeModal'));
      if (modal) modal.hide();
      // Add new option to select
      var select = document.getElementById('type');
      if (select && name) {
        // Remove "Add New..." option if present
        var addNewOption = select.querySelector('option[value="add_new"]');
        if (addNewOption) addNewOption.remove();
        // Add new option
        var option = document.createElement('option');
        option.value = res.unique_id || name;
        option.text = name;
