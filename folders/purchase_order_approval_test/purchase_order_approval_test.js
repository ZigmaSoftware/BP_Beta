...
$(document).ready(function () {
    ...
    $("#item_code").on("change", function () {
        ...
        // ✅ Bind discount type change
    $("#discount_type").on("change", function () {
        handleDiscountTypeChange(); // sets discount field editable/readonly
    });

    // Optional: Trigger once on page load if discount type is preselected
...
function total_amount_calculation() {
    ...
    $("#purchase_order_sub_datatable tbody tr").each(function () {
        ...
        let rate = parseFloat($(this).find("td:eq(5)").text()) || 0;
            if (qty <= 0) {
        console.log("Skipping row due to zero quantity");
        return;  // Skip this iteration
    }
        let discount = parseFloat($(this).find("td:eq(7)").text()) || 0;
        let discount_type = $(this).find("td:eq(6)").text().trim(); // ₹ or %
        let tax_text = $(this).find("td:eq(8)").text();
...
function purchase_order_approval_test_cu(unique_id = "") {
    if (!is_online()) {
...
function po_sub_edit(unique_id) {
    $.ajax({
        ...
        success: function (res) {
            ...
            $("#quantity").val(Math.round(d.appr_quantity));
            $("#uom").val(d.uom);
            $("#rate").val(d.rate);
...
function purchase_order_approval_test_delete(unique_id = "") {
...
function purchase_order_approval_test_upload(unique_id){
