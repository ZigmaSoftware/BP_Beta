...
switch ($action) {
    case 'datatable':
        ...
        // ✅ Columns for Pending srn Report
        $columns = [
            "@a := @a + 1 AS s_no",
            ...
            "FORMAT(COALESCE(srn_sub.max_received_qty, 0), 2) AS received_qty",
            "FORMAT(GREATEST(poi.quantity - COALESCE(srn_sub.max_received_qty, 0), 0), 2) AS pending_qty",
        
            // Amount
            "(poi.quantity * poi.rate) AS amount",
        
            // srn details
            "COALESCE(GROUP_CONCAT(DISTINCT srn.srn_number SEPARATOR '<br>'), '-') AS srn_no",
            "COALESCE(
                GROUP_CONCAT(
                    DISTINCT 
                    CASE 
            ...
            "CASE 
                WHEN srn_sub.has_approved = 0 THEN 'Pending'
                WHEN COALESCE(srn_sub.max_received_qty,0) = 0 THEN 'Pending'
                WHEN poi.quantity <= srn_sub.max_received_qty THEN 'Closed'
                WHEN poi.quantity > srn_sub.max_received_qty THEN 'Partially Received'
                ELSE 'Pending'
            END AS status"
        ...
        $table_details = [
            "purchase_order po
             ...
             LEFT JOIN (
                ...
                FROM srn_sublist gs
                INNER JOIN srn g ON gs.screen_unique_id = g.screen_unique_id
                WHERE gs.is_delete = 0 AND g.is_delete = 0
                GROUP BY gs.item_code
            ) srn_sub ON poi.item_code = srn_sub.item_code",
            $columns
        ];
        ...
        $where = " po.is_delete = 0 
           ...
           AND NOT (
               srn_sub.has_approved = 1 
               AND poi.quantity <= COALESCE(srn_sub.max_received_qty, 0)
           )";
        ...
        if (!empty($status)) {
            if ($status === "Pending") {
                $where .= " AND (
                    srn_sub.has_approved = 0
                    OR COALESCE(srn_sub.max_received_qty,0) = 0
                )";
            } elseif ($status === "Closed") {
                $where .= " AND poi.quantity <= COALESCE(srn_sub.max_received_qty, 0)";
            } elseif ($status === "Partially Received") {
                $where .= " AND poi.quantity > COALESCE(srn_sub.max_received_qty, 0)
                            AND COALESCE(srn_sub.max_received_qty,0) > 0";
            }
        }
