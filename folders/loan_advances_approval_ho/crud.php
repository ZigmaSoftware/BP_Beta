...
switch ($action) {
    case 'createupdate':
        ...
        if($is_approved == 1){
        $columns            = [
            "hod_approval"       => $_POST["is_approved"],
            "hod_approve_by"     => $_SESSION['staff_id'],
            "hod_reason"         => $_POST['hod_reason'],
            "ceo_app_status"     => 1,
            "ceo_to_be_approved" => $_POST['ceo_to_be_approved'],
            "hod_approve_date"   => date('Y-m-d'),
        ];
    ...
    case 'datatable':
        ...
        if ($_SESSION['sess_user_type'] != $admin_user_type) {
            $where = "c.is_delete = '0' AND  b.reporting_officer = '".$_SESSION['staff_id']."' AND c.hod_app_status = 1";
        }else{
            $where = " c.is_delete = '0' AND c.hod_app_status = 1 ";
        }
       ...
       // if (isset($_POST['to_date'])) {
        ...
        if ($result->status) {
            ...
            foreach ($res_array as $key => $value) {
                ...
                switch ($value['hod_approval']) {
                    case "0":
                        $text = '<span class="text-center text-warning">Pending</span>';
                        break;
                    case "1":
                        $text = '<span class="text-center text-success">CEO Approval Required</span>';
                        break;
                    case "2":
