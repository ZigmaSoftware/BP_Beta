...
switch ($action) {
    case 'createupdate':
        ...
        $columns            = [
            ...
            "accept_resig_date"             =>$_POST['accept_resig_date'],
        ...
        if ($unique_id) {
            ...
            // Insert Begins 
            $where_resignation                  = "acc_year = '" . $_SESSION["acc_year"] . "'";

            $resignation_no                     = bill_no($table, $where_resignation, "XWM/HR/RES/", 1, 0, 0, false, "/");

            $columns["resignation_acceptance_letter_no"]   = $resignation_no;
    ...
    case 'datatable':
        ...
        if ($result->status) {
            ...
            foreach ($res_array as $key => $value) {
                ...
                $btn_view                   = btn_print1($folder_name, $value['unique_id'], 'resignation_acceptance_letter',"","","");
                $btn_update                 = btn_update($folder_name, $value['unique_id']);
                $btn_delete                 = btn_delete($folder_name, $value['unique_id']);
                $value['unique_id']         = $btn_view . $btn_update . $btn_delete;
                $value['entry_date'] = '<b>' . $value['resignation_acceptance_letter_no'] . '</b><br>' . $value['entry_date'];
    ...
    case 'get_staffdetails':
        ...
        $table_columns = [
            "staff_name",
            "employee_id",
            "(SELECT company_name FROM company_name_creation AS company_name_creation WHERE company_name_creation.unique_id = " . $table_name . ".company_name ) AS company_name",
