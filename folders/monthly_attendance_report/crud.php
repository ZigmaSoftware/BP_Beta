...
switch ($action) {
    ...
    case 'fullday_leave_report_datatable':
        ...
        if($length == '-1') {
            ...
            $total_staff    = get_total_staff($entry_date);
            ...
            $present_staff  = get_present_count($entry_date);   
...
// echo $result->sql;
    ...
    $data       = [];
    $from_date  = $_POST['entry_date'];  // Now acts as from_date
    ...
    $table_details = [
        "view_bp_attendance_test, (SELECT @a:= " . $start . ") AS a",
        $columns
    ];
    ...
    if ($to_date === date('Y-m-d')) {
        // Include records up to today, including those without exit punch
        $where = "punch_date BETWEEN '$from_date' AND '$to_date' 
                  AND ((worked_hours >= '06:00:00' AND exit_punch IS NOT NULL) 
                  OR exit_punch IS NULL)";
    } else {
        // Normal condition for historical data
        $where = "punch_date BETWEEN '$from_date' AND '$to_date' 
    ...
    $order_by       = "punch_date ASC, employee_id ASC";
    $sql_function   = "SQL_CALC_FOUND_ROWS";
...
$result         = $pdo->select($table_details, $where, $limit, $start, $order_by, $sql_function);
    ...
    if ($result->status) {
        ...
        foreach ($res_array as $key => $value) {
            // Format entry and exit punch times
            ...
            // Set worked hours correctly
            $worked_hours = ($value['exit_punch'] === NULL) ? "-" : ($value['worked_hours'] ?? "-");

            $employee_id = strtoupper($value['employee_id']);

            // Prepare data in the correct format (array of arrays)
            $data[] = [
                $s_no++,
                $value['punch_date'],
                $employee_id,
    ...
    case 'absent_staff_report_datatable':
        ...
        ;
        $table_details  = [
            "absent_staff_view_test , (SELECT @a:= ".$start.") AS a ",
            $columns
        ];

        $where = 'punch_date = "' . $entry_date . '" OR absence_reason = "Absent (Not Registered)"';


        $order_by       = " employee_id ";
        // $group_by       = " staff.unique_id ";
        $sql_function   = "SQL_CALC_FOUND_ROWS";
