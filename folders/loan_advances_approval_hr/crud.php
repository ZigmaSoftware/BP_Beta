...
switch ($action) {
    case 'createupdate':
        ...
        if($is_approved == 1){
        $columns            = [
            "hr_approval"           => $_POST["is_approved"],
            "hr_approve_by"         => $_SESSION['staff_id'],
            "hr_reason"             => $_POST['hr_reason'],
            "accounts_app_status"   => 1,
    ...
    case 'datatable':
        ...
        if ($_SESSION['sess_user_type'] != $admin_user_type) {
            $where = "c.is_delete = '0' AND  b.reporting_officer = '".$_SESSION['staff_id']."' AND c.hr_app_status = 1";
        }else{
            $where = " c.is_delete = '0' AND c.hr_app_status = 1 ";
        }
       ...
       // if (isset($_POST['to_date'])) {
        ...
        if ($result->status) {
            ...
            foreach ($res_array as $key => $value) {
                ...
                switch ($value['hr_approval']) {
                    case "0":
                        $text = '<span class="text-center text-warning">Pending</span>';
                        break;
                    case "1":
                        $text = '<span class="text-center text-success">Accounts Approval Required</span>';
                        break;
                    case "2":
