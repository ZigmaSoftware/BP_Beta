...
switch ($action) {
    case 'createupdate':
        ...
        if($is_approved == 1){
        $columns            = [
            "ceo_approval"       => $_POST["is_approved"],
            "ceo_approve_by"     => $_SESSION['staff_id'],
            "ceo_reason"         => $_POST['ceo_reason'],
            "hr_app_status"      => 1,
            "ceo_approve_date"   => date('Y-m-d'),
        ];
    ...
    case 'datatable':
        ...
        if ($_SESSION['sess_user_type'] != $admin_user_type) {
            $where = "c.is_delete = '0' AND  b.reporting_officer = '".$_SESSION['staff_id']."' AND c.ceo_app_status = 1";
        }else{
            $where = " c.is_delete = '0' AND c.ceo_app_status = 1 ";
        }
            ...
            foreach ($res_array as $key => $value) {
                ...
                switch ($value['ceo_approval']) {
                    ...
                    case "1":
                        ...
                        break;
                    case "2":
                        $text = '<span class="text-center text-success">Director Approval Required</span>';
                        break;
                    case "3":
