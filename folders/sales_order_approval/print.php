...
if (isset($_GET['unique_id']) && !empty($_GET['unique_id'])) {
    ...
    if ($main_result->status && !empty($main_result->data)) {
        ...
        $revision_date          = $main_data["revision_date"];
...
$company_name_is       = company_name($company_id);
...
$so_type_name = '';
if($so_type === '1'){
    $so_type_name = 'Product';
} elseif($so_type === '2'){
    $so_type_name = 'Project';
} elseif($so_type === '3'){
    $so_type_name = 'Spare';
} else {
    $so_type_name = 'Maintainance';
}
...
$items = [];
$sub_table = "sales_order_sublist"; // adjust if table name differs
...
if ($result->status) {
    foreach ($result->data as $row) {
        ...
        $row['unit_name']    = $unit_data[0]['unit_name'] ?? $row['unit_name'];

        $tax_data            = tax($row['tax_id']);
        $tax_value            = tax($row['tax_id'])[0]['tax_value'];
        $row['tax_id']       = $tax_data[0]['tax_name'] ?? $row['tax_id'];

        $total_basic  += ((float) $row['rate'] * $row['quantity']);
        $total_amount += (float) $row['amount'];
        $total_gst    += (((float) $row['rate'] * $row['quantity']) * (float) $tax_value) / 100;
...
<div class="invoice-box">
  ...
  <div class="row header-box">
    <div class="col-2 mt-4">
      <img src="/blue_planet_beta/uploads/company_creation/<?= $company_name_is[0]['logo'] ?>" alt="Company Logo" style="max-height:60px;">
  </div>
   ...
   <div class="col-6 company-info">
    ...
    <div class="col-4 po-details text-wrap text-break">
      <h2>Sales Order</h2>
      <p class="mb-0">
        <span style="font-size:20px; font-weight:600; color:#03407f; word-break:break-word; white-space:normal;">
        <?= $sales_order_no ?>     </span><br>
        <strong>Date:</strong> <?= $entry_date ?>     </p>
    </div>
  </div>
...
<div class="backgrinfo"><h6>Customer Details </h6></div>
       <p class="mb-0"><strong><?=  $supplier_data[0]['customer_name'] ?></strong></p>
        ...
        <p class="mb-0">GST No: <?= !empty($supplier_data[0]['gst_no']) ? htmlspecialchars($supplier_data[0]['gst_no']) : '<strong>  -  </strong>' ?><br>
          ...
          Contact Person:  <?=   $contact_person_name ?? "-" ?><br>Contact Number: <?= $supplier_contacts[0]['contact_person_contact_no'] ?? '<strong>  -  </strong>' ?><br>
...
<!--       <p class="mb-0"><strong>A016 - AGASTYA INVENTION PVT. LTD</strong></p>-->
<!--      <p class="mb-0">H.O. 1D SATYAM I460, RAFI AHMED KIDWAI ROAD, KOLKATA-700016-->
<!--Kolkata - 700016</p>-->
<!--        <p class="mb-0">-->
<!--          GST No: 19AAUCA5883E1Z2,<br>-->
<!--          PAN No: AAUCA5883E<br></p>-->
          
<!--           <p class="mb-0"><b>Warehouse</b> : STR-X001 / Xeon Store </p>-->
<!--           <p class="mb-0"><b>Contact Person</b>: Test</p>-->
<!--           <p class="mb-0"><b>Contact No</b>: 1234567890</p>-->
...
<!--    </div>-->
    ...
    <div class="col-6">
      ...
      <table class="table table-sm table-borderless">
        ...
        <!--<tr><td>SO Type:</td><td><//?= $so_type_name ?? "-" ?></td></tr>-->
        <tr><td>Cust PO No:</td><td><?= $customer_po_no ?? "-" ?></td></tr>
        <tr><td>Cust PO Date:</td><td><?= $customer_po_date ?? "-" ?></td></tr>
        <!--<tr><td>Incoterms:</td><td><//?= $incoterms ?? "-" ?></td></tr>-->
        <!--<tr><td>End User:</td><td><//?= $enduser ?? "-" ?></td></tr>-->
        <tr><td>Project Code:</td><td><?= $sales_order_no ?? "-" ?></td></tr>
...
<tbody>
  <?php if (!empty($items)): ?>
      <?php foreach ($items as $item): ?>
          <tr>
              <td><?= $item['s_no'] ?></td>
              <td><?= htmlspecialchars($item['item_name_id']) ?></td>
              <td><?= htmlspecialchars($item['unit_name']) ?></td>
              <td class="text-right"><?= number_format($item['quantity'], 2) ?></td>
              <td class="text-right"><?= number_format($item['rate'], 2) ?></td>
              <td><?= htmlspecialchars($item['tax_id']) ?></td>
              <td class="text-right"><?= number_format($item['amount'], 2) ?></td>
              <td><?= htmlspecialchars($item['subtask']) ?></td>
          </tr>
      <?php endforeach; ?>
  <?php else: ?>
  ...
  <table class="table table-borderless totals">
    <tr>
      <td class="text-right">Basic Value:</td>
    ...
    <tr>
      <td class="text-right">Total Value (INR):</td>
      <td class="text-right"><?= number_format($total_amount, 2) ?></td>
    </tr>
  </table>

  <p><strong>INR : TWENTY RUPEES AND TEN SIX PAISE ONLY</strong></p>

  <!-- Signatures -->
  <div class="row mt-4">
    <div class="col-7"></div>
    <div class="col-5 text-center">
      <p><strong>For, <?= $company_name_is[0]['company_name'] ?></strong></p>
      <div class="row">
...
<div class="text-center no-print mt-3">
  <button class="btn btn-primary" onclick="window.print()">Print Sales Order</button>
</div>
...
<!--<br>-->
<!--<div class="col-sm-6">-->
<!--    <div class="title-logo text-start" style="margin-left:0; display:flex; align-items:center; justify-content:flex-start;">-->
<!--        <div class="logo">-->
<!--            <a href="#">-->
<!--                <img src="https://zigma.in/blue_planet_beta/assets/images/logo.png" -->
<!--                     alt="Company Logo" style="height: 60px; display:block;">-->
<!--            </a>-->
<!--        </div>-->
<!--    </div>-->
...
<!--    <p><b>BLUE PLANET INTEGRATED WASTE SOLUTIONS LIMITED,</b></p>-->
<!--      <p>-->
<!--        Kohinoor World Towers T3, Office No.306, Opp. Empire Estate,<br />-->
<!--Old Mumbai-Pune Hwy, Pimpri Colony, Pune Maharashtra-411018.<br>-->
<!--         Email: -<br>-->
<!--      </p> -->
...
<!--</div>-->
<!--<div class="col-sm-6">-->
<!--    <div class="title-logo text-end" style="margin-left:400px">-->
<!--        <div class="logo">-->
<!--            <a href="#">-->
...
<!--        <td  > -->
<!--            <div class="col-lg-12 p-0">-->
<!--                <div class="invoice-inner-2" style="margin-top:-40px">-->
<!--                    <div class="invoice-info" id="invoice_wrapper">-->
<!--                        <div class="invoice-inner" >-->
<!--                            <div class="invoice-top">-->
<!--                                <div class="row align-items-center">-->
<!--                                    <div class="col-sm-12 termcondi" >-->
<!--                                        <p style="font-size:10px">ANNEXURE-A</p>-->
<!--<p style="font-size:9px">ANTI-BRIBERY AND CORRUPTION AND ANTI-MONEY LAUNDERING POLICY</p>-->
...
<!--<tr>-->
<!--   <th><b>1.5</b></th>-->
<!--   <td class="space_rt mb-4">Bribes could include the following examples:-->
...
<!--<tr>-->
<!--   <th><b>1.6</b></th>-->
<!--   <td class="space_rt">Facilitation payments are strictly prohibited. These include any irregular one-time and/or routine payments made to government officials to expedite or secure routine governmental action. Any such payments must immediately be brought to the attention of the Chief Compliance Officer <b>(“CCO”)</b>  or Compliance Officer<b> (“CO”),</b>  as applicable to a Blue Planet Group entity. The details of such persons are as provided below:-->
   
<!--<table class="table_border mt-0" border="1" cellspacing="0" cellpadding="0" width="100%">-->
<!--  <tr>-->
<!--    <td width="130" valign="top"><p><strong>Name </strong></p></td>-->
<!--    <td width="125" valign="top"><p><strong>Designation</strong></p></td>-->
<!--    <td width="184" valign="top"><p><strong>Email    Address</strong></p></td>-->
<!--    <td width="156" valign="top"><p><strong>Location</strong></p></td>-->
<!--  </tr>-->
<!--  <tr>-->
<!--    <td width="130" valign="top"><p>Mr. Dhananjay Jitesh Pandey</p></td>-->
<!--    <td width="125" valign="top"><p>Group Compliance Officer</p></td>-->
<!--    <td width="184" valign="top"><p><a href="mailto:dhananjay.pandey@blue-planet.com">dhananjay.pandey@blue-planet.com</a></p></td>-->
<!--    <td width="156" valign="top"><p>New Delhi, India</p></td>-->
<!--  </tr>-->
<!--</table>-->
<!--</td>-->
<!--</tr>-->
...
<!-- <tr>-->
<!--   <th><b>1.7</b></th>-->
<!--   <td class="space_rt">No person who is subject to this Policy shall: -->
<!--   <ol class="d sublist_or">-->
...
<!--<tr>-->
<!--   <th><b>1.10</b></th>-->
<!--   <td class="space_rt">Examples of government and public officials include: -->
<!--   <ol class="d">-->
...
<!--<tr>-->
<!--   <th><b>1.11</b></th>-->
<!--   <td class="space_rt"><b>Compliance</b> -->
<!--   <ol class="d">-->
...
<!--       <li>	The CCO or the CO of the Company shall ensure that: -->
<!--         <ol type="I">-->
...
<!--       </li>-->
<!--        <li>Specific guidance on common forms of bribery:-->
<!--          <ol type="I">-->
...
<!--              <li> <u>Political contributions:</u> Covered Persons must not use the Company’s resources including funds or facilities to provide support for, or contribute to any political organisation or candidate as the Blue Planet Group is strictly apolitical.-->
<!--</li>-->
<!--          </ol>-->
...
<!--<tr> <th> <b>1.12</b></th>-->
<!--<td class="space_rt"><b>Reporting</b>-->
...
<!--    <li> 	Employees who report potential misconduct in good faith or who provide information or otherwise assist in any inquiry or investigation of potential misconduct will be protected against retaliation.-->
<!--</li>-->
<!--</ol>-->
<!--</td>-->
...
<!--    <tr>-->
<!--   <th><b>2.2</b></th>-->
<!--   <td class="space_rt"> <b>Meals, Entertainment, Travel and Lodging </b>-->
<!--     <ol class="d">-->
...
<!--              </li>-->
<!--              <li>	Money laundering usually consists of 3 components -->
<!--                   <ol type="i"> -->
...
<!--                   <li>	Some examples of red flags to be reported include: -->
<!--                     <ol type="i">-->
