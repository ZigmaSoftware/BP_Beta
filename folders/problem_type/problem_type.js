...
var form_name = "problem_type";
var table_id  = "problem_datatable";
...
// 💾 CREATE / UPDATE
// ==========================
function problem_type_cu() {
    let formdata = new FormData($("#problem_type_form")[0]);
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (res.status == 1) {
                Swal.fire({
                    title: "Success",
                    text: res.error || "problem Type saved successfully!",
                    icon: "success",
                ...
                Swal.fire({
                    title: "Failed",
                    text: res.error || "Unable to save Problem Type.",
                    icon: "error",
                    confirmButtonText: "OK"
                });
            }
...
function init_datatable(table_id = "", form_name = "", action = "", filter_data = {}) {
    ...
    table.DataTable({
        ...
        columns: [
            ...
            { data: "problem_type", title: "Problem Type Name" },
        ...
        responsive: true,
        language: {
            emptyTable: "No Problem Types found"
        }
    });
}
...
// ==========================
function problem_type_toggle(unique_id, current_status) {
    if (!unique_id) {
        Swal.fire({
    ...
    Swal.fire({
        title: `${actionText} Problem Type?`,
        text: `Are you sure you want to ${actionText.toLowerCase()} this Problem Type?`,
        icon: "question",
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (res.status == 1) {
                        Swal.fire({
                            title: "Success",
                            text: res.error || `Problem Type ${actionText.toLowerCase()}d successfully.`,
                            icon: "success",
                            timer: 1500,
                        ...
                        Swal.fire({
                            title: "Failed",
                            text: res.error || "Unable to update Problem Type status.",
                            icon: "error",
                            confirmButtonText: "OK"
                        });
                    }
