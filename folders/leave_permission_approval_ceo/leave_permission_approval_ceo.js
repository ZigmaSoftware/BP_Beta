...
function datatable_init_based_on_prev_state () {
		...
		var from_date  	    = sessionStorage.getItem("leave_ceo_from");
		var to_date         = sessionStorage.getItem("leave_ceo_to");
		var filter_action   = sessionStorage.getItem("leave_ceo_action");
	
		if (!from_date) {
			from_date = $("#leave_ceo_from").val();
		} else {
			$("#leave_ceo_from").val(from_date);
		}
	
		if (!to_date) {
			to_date = $("#leave_ceo_to").val();
		} else {
			$("#leave_ceo_to").val(to_date);
		}
...
function leave_permission_approval_ceo_cu  (unique_id = "") {
...
function leavepermissionceoFilter(filter_action = 0) {
	var internet_status = is_online();
	...
	if (is_form) {
		var from_date       = $("#leave_ceo_from").val();
		var to_date         = $("#leave_ceo_to").val();
		
		var is_vaild = fromToDateValidity(from_date, to_date);

		if (is_vaild) {

			sessionStorage.setItem("leave_ceo_from_date", from_date);
			sessionStorage.setItem("leave_ceo_to_date", to_date);
			sessionStorage.setItem("leave_ceo_action", filter_action);
