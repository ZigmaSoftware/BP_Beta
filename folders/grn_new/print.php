...
function convert_number_to_words($number) {
    $no = floor($number);
    $point = round($number - $no, 2) * 100; // for paise
    $hundred = null;
    $digits_1 = strlen($no);
    $i = 0;
    $str = [];
...
if ($city_values->status) {
    ...
    $created_user_id        = $city_values[0]["created_user_id"];
}

$po_data = po_data($purchase_number)[0];
// print_r($po_data);
...
$company_name_is       = company_name($company_name);
...
$supplier_data = supplier_data($supplier_name);
// print_r($supplier_data);

$city1 = city($supplier_data[0]['city'])[0]['city_name'];
$state1 = state($supplier_data[0]['state'])[0]['state_name'];
$country1 = country($supplier_data[0]['country'])[0]['country_name'];

$supplier_contacts = supplier_contact_data($supplier_name);
...
// print_r($project_options[0]);
// $project_options  = select_option($project_options,"Select the Project Name",$project_id);
// Fetch project creation details for received address
$project_details = $pdo->select(
    ["project_creation", ["address", "city", "state", "country", "pin_code"]],
    ["unique_id" => $project_id, "is_delete" => 0]
);

$received_address = "-";
if ($project_details->status && !empty($project_details->data)) {
    $proj = $project_details->data[0];
    // Optional: format nicely with city/state/country
    $city_proj    = city($proj['city'])[0]['city_name'] ?? "";
    $state_proj   = state($proj['state'])[0]['state_name'] ?? "";
    $country_proj = country($proj['country'])[0]['country_name'] ?? "";
    $received_address = $proj['address'] . ", " . $city_proj . ", " . $state_proj . ", " . $country_proj . " - " . $proj['pin_code'];
}
    ...
    "ROUND(
        CASE 
            ...
            WHEN poi_items.discount_type = 2 THEN 
                ((COALESCE(gs.update_qty, 0) * COALESCE(poi_items.rate, 0)) 
                - COALESCE(poi_items.discount, 0)) * 1.12
            ELSE 
...
$table_items = [
    "$sub_table gs 
        LEFT JOIN ( 
            SELECT gs2.item_code, gs2.po_unique_id, SUM(gs2.update_qty) AS total_received_qty
            FROM grn_sublist AS gs2
            LEFT JOIN grn AS g ON g.screen_unique_id = gs2.screen_unique_id
            WHERE gs2.po_unique_id = '{$grn_data['po_number']}' 
...
$result_items = $pdo->select($table_items);
...
$base_amount = 0;
$base_tax = 0;
if ($result_items->status) {
    foreach ($result_items->data as $row) {
        // print_r($row);
        $item_data = item_name_list($row["item_code"]);
        $row["item_name"] = $item_data[0]["item_name"] . " / " . $item_data[0]["item_code"];
        ...
        $tax_value = (float)$tax_data[0]['tax_value'];   // e.g. 28 for 28%
        $row["tax"] = $tax_name;

        // Numbers
        $qty            = (float)$row["update_qty"];     // received qty used for billing
        $rate           = (float)$row["rate"];
        $discount       = (float)$row["discount"];
        $discount_type  = $row["discount_type"];         // 0/1/2
        ...
        } elseif ($discount_type == 2) {     // flat amount
            $disc_amt = $discount;
        } else {
            $disc_amt = 0;
        }

        $after_discount = max($base - $disc_amt, 0);
        
        $base_amount +=  $after_discount;

        // Tax & final amount
        $item_tax = round($after_discount * ($tax_value / 100), 2);
        
        $base_tax += $item_tax;
        
        $row["amount"] = round($after_discount + $item_tax, 2);  // <-- now 1,280.00 for 28%

        // For display label
        $row["discount_type"] = ($discount_type == 1 ? 'Percentage' : ($discount_type == 2 ? 'Amount' : 'No Discount'));
...
<div class="invoice-box">
    ...
    // PAF, Freight, Other, Round values (set 0 if empty)
    // Default values if not set
    $paf         = $paf ?? 0;
    $gst_paf     = $gst_paf ?? 0;    // This is a TAX ID, not amount
    $freight     = $freight ?? 0;
    $gst_freight = $gst_freight ?? 0; // TAX ID
    $other       = $other ?? 0;
    $gst_other   = $gst_other ?? 0;   // TAX ID
    $round       = $round ?? 0;
    
    // --- 1. Get PAF GST Percentage ---
    $paf_gst_value = 0;
    if (!empty($gst_paf)) {
        $paf_tax_data = tax($gst_paf);
        $paf_gst_value = isset($paf_tax_data[0]['tax_value']) ? (float)$paf_tax_data[0]['tax_value'] : 0;
    }
    $paf_tax_amount = ($paf * $paf_gst_value) / 100;
    $total_paf = $paf + $paf_tax_amount;
    
    // --- 2. Get Freight GST Percentage ---
    $freight_gst_value = 0;
    if (!empty($gst_freight)) {
        $freight_tax_data = tax($gst_freight);
        $freight_gst_value = isset($freight_tax_data[0]['tax_value']) ? (float)$freight_tax_data[0]['tax_value'] : 0;
    }
    $freight_tax_amount = ($freight * $freight_gst_value) / 100;
    $total_freight = $freight + $freight_tax_amount;
    
    // --- 3. Get Other GST Percentage ---
    $other_gst_value = 0;
    if (!empty($gst_other)) {
        $other_tax_data = tax($gst_other);
        $other_gst_value = isset($other_tax_data[0]['tax_value']) ? (float)$other_tax_data[0]['tax_value'] : 0;
    }
    $other_tax_amount = ($other * $other_gst_value) / 100;
    $total_other = $other + $other_tax_amount;
    
    // --- 4. Total Additional Charges ---
    $total_additional_charges = $total_paf + $total_freight + $total_other;
    
    // Add PAF, Freight & Other GSTs to total tax
    $total_tax += $base_tax + $paf_tax_amount + $freight_tax_amount + $other_tax_amount;
    
    // Final Grand Total Calculation
    $grand_total = $total_amount + $total_additional_charges + $round;
    ?>
  ...
  <div class="row header-box">
    <div class="col-2 mt-2">
      <img src="/blue_planet_beta/uploads/company_creation/<?= $company_name_is[0]['logo'] ?>" alt="Company Logo" style="width:100%">
    </div>

    <div class="col-6 company-info">
      <h4><?= $company_name_is[0]['company_name'] ?></h4>
      <p>
        <?= $company_name_is[0]['address'] ?><br>
        GST No: <?= htmlspecialchars($company_name_is[0]['gst_number']) ?> | 
        PAN No: <?= htmlspecialchars($company_name_is[0]['pan_number']) ?>
      </p>
    </div>

    <div class="col-4 po-details text-wrap text-break">
      <h2>GRN</h2>
      <p class="mb-0">
        <span style="font-size:18px; font-weight:600; color:#03407f; word-break:break-word; white-space:normal;">
          <?= htmlspecialchars($grn_number) ?>
        </span><br>
        <strong>Date:</strong> <?= htmlspecialchars($entry_date) ?>
      </p>
    </div>
  </div>
  ...
  <div class="row mb-2">
    <div class="col-6">
      <div class="backgrinfo"><h6>Supplier Details</h6></div>
        <p class="mb-0"><strong><?=  $supplier_data[0]['supplier_name'] ?></strong></p>
        <p class="mb-0"><?=  $supplier_data[0]['address'] . ' ' . $city . ', ' . $state . ', ' . $country . ' - ' . $supplier_data[0]['pincode'] . '.' ?></p>
        <p class="mb-0">GST No: <?= !empty($supplier_data[0]['gst_no']) ? htmlspecialchars($supplier_data[0]['gst_no']) : '<strong>  -  </strong>' ?><br>
          PAN No: <?= $supplier_data[0]['pan_no'] ?: '-'; ?><br>
          Contact Person:  <?=   $supplier_contacts[0]['contact_person_name'] ?? "-" ?><br>Contact Number: <?= $supplier_contacts[0]['contact_person_contact_no'] ?? '<strong>  -  </strong>' ?><br>
          E-Mail: <?= $supplier_contacts[0]['contact_person_email'] ?? '<strong>  -  </strong>' ?></p>
    ...
    <div class="col-6">
      ...
      <div class="mb-2">
  <div class="row mb-1">
    <div class="col-4 fw-bold">Doc No:</div>
    <div class="col-8"><?= htmlspecialchars($doc_no) ?></div>
  </div>
  <div class="row mb-1">
    <div class="col-4 fw-bold">DC No:</div>
    <div class="col-8"><?= htmlspecialchars($dc_no) ?></div>
  </div>
  <div class="row mb-1">
    <div class="col-4 fw-bold">Invoice No:</div>
    <div class="col-8"><?= htmlspecialchars($supplier_invoice_no) ?></div>
  </div>
  <div class="row mb-1">
    <div class="col-4 fw-bold">Invoice Date:</div>
    <div class="col-8"><?= htmlspecialchars($invoice_date) ?></div>
  </div>
  <div class="row mb-1">
    <div class="col-4 fw-bold">PO Number:</div>
    <div class="col-8"><?= htmlspecialchars($po_data['purchase_order_no']) ?></div>
  </div>
  <div class="row mb-1">
    <div class="col-4 fw-bold">PO Date:</div>
    <div class="col-8"><?= htmlspecialchars($po_data['entry_date']) ?></div>
  </div>
  <!--
  <div class="row mb-1">
    <div class="col-4 fw-bold">Contact Person:</div>
    <div class="col-8"><?= htmlspecialchars($contact_person_name) ?></div>
  </div>
  <div class="row mb-1">
    <div class="col-4 fw-bold">Contact No:</div>
    <div class="col-8"><?= htmlspecialchars($contact_person_no) ?></div>
  </div>
  -->
  <div class="row mb-1">
    <div class="col-4 fw-bold">Project:</div>
    <div class="col-8"><?= $project_options[0]['label'] ?></div>
  </div>
...
<!-- Items Table -->
  <div class="no-overflow">
        <table class="table table-bordered table-striped table-hover table-sm mb-2" style="width:100%;">
            ...
            <tbody>
                <?php foreach ($items as $i => $row): ?>
                    <tr>
                        ...
                        <td class="text-end"><?= $row["order_qty"] ?></td>
                        <td><?= $row["uom"] ?></td>
                        <td class="text-end"><?= $row["update_qty"] ?></td>
        ...
        <table class="table table-borderless totals">
            <tr>
                <td class="text-end">Basic Value:</td>
                <td style="width: 150px;" class="text-end"><?= number_format($base_amount, 2) ?></td>
            </tr>
            <tr>
                <td class="text-end"><strong>Total Tax:</strong></td>
                <td class="text-end"><strong><?= number_format($base_tax, 2) ?></strong></td>
            </tr>
            <tr>
        ...
        <?php
            $total_in_words = strtoupper(convert_number_to_words($grand_total, 2));
            if (empty(trim($total_in_words))) {
                $total_in_words = "ZERO";
            }
        ?>
        <p><strong>Total in Words : <?= strtoupper($total_in_words) ?> ONLY</strong></p>
    </div>

<h5 style="margin-top:15px; font-weight:600;">Transport Details:</h5>
<div class="row mb-1">
  <div class="col-4 fw-bold">Eway Bill No:</div>
  <div class="col-8"><?= !empty($eway_bill_no) ? htmlspecialchars($eway_bill_no) : '-' ?></div>
</div>

<div class="row mb-1">
  <div class="col-4 fw-bold">Eway Bill Date:</div>
  <div class="col-8">
    <?= (!empty($eway_bill_date) && $eway_bill_date != '0000-00-00' && $eway_bill_date != '0000-00-00 00:00:00') 
          ? htmlspecialchars($eway_bill_date) 
          : '-' ?>
  </div>
  <div class="row mb-1">
  <div class="col-4 fw-bold">Received Address:</div>
  <div class="col-8"><?= htmlspecialchars($received_address) ?></div>
</div>
  ...
  <div class="row mt-2">
    ...
    <div class="col-5 text-center">
      <p><strong>For, <?= htmlspecialchars($company_name_is[0]['company_name']) ?></strong></p>
      <div class="row">
        <div class="col-6 mt-2">
          <p><strong>Prepared By</strong><br><?= htmlspecialchars($created_user_id) ?></p>
        </div>
        <div class="col-6 mt-2">
          <p><strong>Authorised By</strong><br><?= htmlspecialchars($approved_by) ?></p>
        </div>
      </div>
    </div>
  </div>
