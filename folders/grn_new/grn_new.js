...
function grn_new_cu(unique_id = "") {
    ...
    let data = new FormData($("#grn_new_form")[0]);
...
// Info Function
function grn_new_info(unique_id = "") {
    if (!unique_id) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if ((status === true || status === 1) && rows.length > 0) {
                ...
                `;
                $("#grnInfoModalBody").append(iframeHtml);

                // Open modal
                $('#grnInfoModal').modal('show');
            } else {
