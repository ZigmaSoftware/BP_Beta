...
var form_footer 	= '';
var table_name 		= '';
var table_id 		= 'staff_accounts_report_datatable';
var action 			= "datatable";

function datatable_init_based_on_prev_state () {
		// Data Table Filter Function Based ON Previous Search
		var department_name  	    = sessionStorage.getItem("department_name");
...
function staffaccountFilter(filter_action = 0) {
	var internet_status = is_online();
	...
	if (is_form) {
		...
		if (is_vaild) {
			...
			sessionStorage.setItem("department_name", department_name);
