...
if ($result->status) {
    foreach ($result->data as $row) {
        ...
        if ($row["lvl_2_status"] == 0) {
            $final_status = 0; // pending overrides everything
        } elseif ($row["lvl_2_status"] == 2 && $final_status != 0) {
            $final_status = 2; // rejected if not already pending
        }
...
<div class="invoice-box">
   ...
   <div class="col-6 company-info">
    ...
    <div class="col-4 po-details text-wrap text-break">
      ...
      <p class="mb-0">
        ...
        <?= $pr_number ?>     </span><br>
        <strong>Date:</strong> <?= $requisition_date ?>     </p>
    </div>
  </div>
  ...
  <div class="row mb-2">
            ...
            <div class="col-6">
               <div class="backgrinfo">
                  <h6>PR Details</h6>
               </div>
               <table class="table table-sm table-borderless">
                   <tr><td>PR No:</td><td><?= $pr_number ?? "-" ?></td></tr>
                  <tr><td>SO No:</td><td><?= $sales_order_no ?? "-" ?></td></tr>
                  <tr><td>Type:</td><td><?= $requisition_type_text ?? "-" ?></td></tr>
                  <tr><td>Date :</td><td><?= $requisition_date ?? "-" ?></td></tr>
   ...
   <table class="table table-bordered mb-1">
      <thead class="thead-light">
         <tr>
            <th style="width:5%;">#</th>
            <th style="width:45%;">Item Name/Code</th>
            <th>Qty</th>
            <th>UOM</th>
            <th>Description</th>
            <th>Remarks</th>
      ...
      <?php if (!empty($data)): ?>
          <?php foreach ($data as $item): ?>
              ...
              <tr>
                  ...
                  <td><?= $item['remarks'] ?></td>
                  <td>
                    <?= (!empty($item['req_date']) && $item['req_date'] !== "0000-00-00") 
                        ? $item['req_date'] 
                        : "-" ?>
                  </td>
              </tr>

              <!-- Sublist Rows (No Desc/Delivery Date) -->
              <?php if (!empty($item['sublist'])): ?>
                  <?php foreach ($item['sublist'] as $sub): ?>
                      <tr class="table-secondary">
                          <td><?= $sub['sno'] ?></td>
                          <td><?= $sub['item'] ?></td>
                          <td><?= $item['quantity'] * $sub['qty'] ?></td>
                          <td><?= $sub['uom'] ?></td>
                          <td colspan="3"><?= $sub['remarks'] ?></td>
                      </tr>
                  <?php endforeach; ?>
              <?php endif; ?>
          <?php endforeach; ?>
