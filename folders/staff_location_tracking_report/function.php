...
function get_break_in_time($staff_id,$entry_date){
    global $pdo;
    $table_name    = "view_staff_break_in_out";
    $where         = [];
    $table_columns = [
        "break_in_time",
    ];
...
// mythili
// get location name from lat and lon
function getAddress($latitude,$longitude)
{
    
    $api_key = "AIzaSyANICEZACihcXoAnBPZS-9hXzNybuQXyZ4";
    
    // Create a URL to request location data from Google Maps
    $url = "https://maps.googleapis.com/maps/api/geocode/json?latlng={$latitude},{$longitude}&key={$api_key}";
  // print_r($url);
    // Send an HTTP request to the Google Maps Geocoding API
    ...
    if ($data->status == "OK" && isset($data->results[0]->formatted_address)) {
     ...
     //print_r($locationName);
       // if ($locationName) {
            // $final_str= '<a href="javascript:void(0);" onclick="openMap(' . $latitude . ', ' . $longitude . ')">'.$locationName.'</a>';
            $final_str = '<a href="javascript:void(0);" onclick="openMap(\''.$locationName.'\')">'.$locationName.'</a>';
        // print_r($locationName);  
           // return $final_str;
        //}
...
function getcheckoutAddress($check_out_latitude,$check_out_longitude)
{
    
    $api_key = "AIzaSyANICEZACihcXoAnBPZS-9hXzNybuQXyZ4";
    
    // Create a URL to request location data from Google Maps
    $url = "https://maps.googleapis.com/maps/api/geocode/json?latlng={$check_out_latitude},{$check_out_longitude}&key={$api_key}";
    ...
    // Check if the response was successful and has results
    if ($data->status == "OK" && isset($data->results[0]->formatted_address)) {
        $checkoutlocationName = $data->results[0]->formatted_address;
    
        if ($checkoutlocationName) {
            
            $final_str_name = '<a href="javascript:void(0);" onclick="checkoutopenMap(\''.$checkoutlocationName.'\')">'.$checkoutlocationName.'</a>';
            // $final_str_name = '<a  href=" href="javascript:void(0)/AIzaSyANICEZACihcXoAnBPZS-9hXzNybuQXyZ4=1&query=onclick="checkoutopenMap(\''.$checkoutlocationName.'\')">'.$checkoutlocationName.'</a>';
            return $final_str_name;
        }
        // Display the location name
        return $checkoutlocationName;
        // print_r($locationName);
    } 
    else {
        return false;
