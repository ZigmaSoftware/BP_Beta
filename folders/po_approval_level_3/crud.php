...
switch ($action) {
    ...
    case "createupdate":
        ...
        $columns = [
            "lvl_3_net_amount"          => $_POST["net_amount"],
            "lvl_3_gross_amount"        => $_POST["gross_amount"],
            
            "lvl_3_status"              => $lvl2_status,
            "lvl_3_reason"              => $_POST["cancelReason"],

            "lvl_3_approved_by"             => $user_id,
            "lvl_3_approved_date"          => $date
        ];
...
break;
    ...
    $columns = [
        ...
        "net_amount",
        "gross_amount",
        "CASE WHEN appr_net_amount IS NULL OR appr_net_amount = 0.00 
              THEN net_amount ELSE appr_net_amount END AS appr_net_amount",
        "CASE WHEN appr_gross_amount IS NULL OR appr_gross_amount = 0.00 
              THEN gross_amount ELSE appr_gross_amount END AS appr_gross_amount",
        "CASE WHEN lvl_2_net_amount IS NULL OR lvl_2_net_amount = 0.00 
              THEN appr_net_amount ELSE lvl_2_net_amount END AS lvl_2_net_amount",
        "CASE WHEN lvl_2_gross_amount IS NULL OR lvl_2_gross_amount = 0.00 
              THEN appr_gross_amount ELSE lvl_2_gross_amount END AS lvl_2_gross_amount",
        "CASE WHEN lvl_3_net_amount IS NULL OR lvl_3_net_amount = 0.00 
              THEN lvl_2_net_amount ELSE lvl_3_net_amount END AS lvl_3_net_amount",
        "CASE WHEN lvl_3_gross_amount IS NULL OR lvl_3_gross_amount = 0.00 
              THEN lvl_2_gross_amount ELSE lvl_3_gross_amount END AS lvl_3_gross_amount",
        "status",         // raw L1
        "lvl_2_status",   // raw L2
        "lvl_3_status",   // raw L3
        "unique_id",
    ...
    // Only show rows eligible for L3 (L2 approved, gross > 10L)
    $where = "is_delete = '0' AND lvl_2_status = '1' AND gross_amount > '1000000'";
    ...
    if ($result->status) {
        foreach ($result->data as $key => $value) {
            ...
            // 🚫 Skip rows not ready for L3
            if ($approval_state == 'pending_l1' || $approval_state == 'pending_l2') {
                continue;
            }

            // Action buttons
            $action_buttons = "";
            if ($approval_state == 'approved_l3' || $approval_state == 'pending_l3') {
