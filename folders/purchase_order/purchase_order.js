...
var sub_datatable = $("#purchase_order_sub_datatable").DataTable({
	"searching": false,
	"paging": false,
	// "ordering": false,
	"info": false,
		...
		if (sublist_data) {
			...
			function insert_arr(values) {
				...
				sub_datatable.row.add([
					...
					`<button type="button" class="addRow btn btn-success ${add_none}"  tabindex="${counter}" >Add</button>`
			...
			sub_datatable.row.add([
				...
				`<button type='button' class='btn btn-danger delete_btn'  tabindex='${counter}' >Delete</button>` +
				`<button type="button" class="addRow btn btn-success"  tabindex="${counter}" >Add</button>`
...
function purchase_order_cu(unique_id = "") {
