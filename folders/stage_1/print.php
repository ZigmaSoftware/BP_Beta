	...
	if (isset($_GET["unique_id"])) {
		if (!empty($_GET["unique_id"])) {
			...
			$columns = [
				...
				"stage_1_status",
				"stage_1_update_date",
                "(select level from view_level_all_departments where unique_id= complaint_creation.unique_id order by unique_id DESC LIMIT 1) as stage"
			];
								...
								foreach ($res_array as $key => $value) {
									...
									<?php }
										...
										if(($cfile_name[1]=='xls')||($cfile_name[1]=='xlsx')) {
											...
											<a href="javascript:print_view('document/<?=$file_name;?>')"><img src="<?=$url;?>"  height="43px" width="53px" >Task Details</a></div></div>
									<?php  }else{ 
...
function print_view(file_name){
    // alert(file_name);
       onmouseover= window.open('../../../g_admin/uploads/complaint_category/'+file_name,'_blank','height=600,width=900,scrollbars=yes,resizable=no,left=200,top=150,toolbar=no,location=no,directories=no,status=no,menubar=no');
    } 
    
function print_view_1(file_name)
{
       onmouseover= window.open('../../../g_admin/uploads/stage_1/'+file_name,'_blank','height=600,width=900,scrollbars=yes,resizable=no,left=200,top=150,toolbar=no,location=no,directories=no,status=no,menubar=no');
    }     
