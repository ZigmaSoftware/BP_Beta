...
$to_date_bf    = date('Y-m',strtotime($to_date));
    ...
    foreach ($team_heads as $head_key => $head_value) {
        ...
        // $where             .= " AND fc.unique_id = '".$forecast_id."' ";
        $fc_where           = " AND fc.unique_id = '".$forecast_id."' ";


        // Get Commited and Achieved By Userwise
        // $sql               = business_forecast_tbody_sql($stage,$where,$commit_where);

        $sql                = "SELECT fc.forecast,'' AS progress,(SELECT IFNULL(SUM(target),0.00) AS target    FROM business_forecast_target WHERE business_forecast_target.is_delete = 0 AND  business_forecast_target.business_forecast=fc.unique_id $commit_where) AS target,IFNULL(SUM(vfp.total),0.00) AS achieved,fc.unique_id FROM forecast AS fc LEFT JOIN view_followup_product AS vfp ON fc.unique_id = vfp.business_forecast $where WHERE fc.is_delete = 0  $fc_where GROUP BY fc.unique_id,vfp.business_forecast";
        ...
        if ($head_result->status) {
            ...
            $head_committed = $head_result['target'];
...
<?php
    ...
    foreach ($team_members_arr as $member_key => $member_value) {
        ...
        $sql                = "SELECT fc.forecast,'' AS progress,(SELECT IFNULL(SUM(target),0.00) AS target    FROM business_forecast_target WHERE business_forecast_target.business_forecast=fc.unique_id  AND business_forecast_target.is_delete = 0  $commit_where) AS target,IFNULL(SUM(vfp.total),0.00) AS achieved,fc.unique_id FROM forecast AS fc LEFT JOIN view_followup_product AS vfp ON fc.unique_id = vfp.business_forecast $where WHERE fc.is_delete = 0  $fc_where GROUP BY fc.unique_id,vfp.business_forecast";
        ...
        if ($member_result->status) {
            ...
            $member_committed = $member_result['target'];
