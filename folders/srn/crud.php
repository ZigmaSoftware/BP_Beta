...
switch ($action) {
    ...
    case "createupdate":
        ...
        $o_invoice              = $_POST["o_invoice"] ?? 0;
        $deli_challan           = $_POST["deli_challan"] ?? 0;
        $test_cert              = $_POST["test_cert"] ?? 0;
        $om_manual              = $_POST["om_manual"] ?? 0;
        $amd_no                 = $_POST['amd_no'];
        $cost_center            = $_POST['cost_center'];
        $deli_challan_no        = $_POST['deli_challan_no'];
        $tax_invoice_no         = $_POST['tax_invoice_no'];
        $tax_invoice_date       = $_POST['tax_invoice_date'];
        $paf                    = $_POST["paf"];
        ...
        $labelData = [];
        $labelData = fetch_srn_number($table);
        $labeldocs = [];
        $labeldocs = fetch_doc_number($table);
        ...
        if (!empty($_POST["unique_id"])) {
            // If unique_id exists, fetch the existing srn_number for this unique_id
            $existing_srn = $pdo->select([$table, ["srn_number"]], ['unique_id' => $_POST["unique_id"], 'is_delete' => 0]);
            $existing_doc = $pdo->select([$table, ["doc_no"]], ['unique_id' => $_POST["unique_id"], 'is_delete' => 0]);
            
            if ($existing_srn->status && !empty($existing_srn->data[0]['srn_number'])) {
                $srn_number = $existing_srn->data[0]['srn_number'];
            } else {
            // Fallback: generate a new srn if not found (should not happen in normal update)
                $srn_number = generatesrn($company_name, $labelData);
            }
            
            if ($existing_doc->status && !empty($existing_doc->data[0]['doc_no'])) {
                $srn_doc = $existing_srn->data[0]['doc_no'];
            } else {
            // Fallback: generate a new srn if not found (should not happen in normal update)
                $srn_doc = generateDoc($labelData);
            }
        } else {
            // No unique_id, so generate a new srn number
            $srn_number = generatesrn($company_name, $labelData);
            $srn_doc = generateDoc($labelData);
        }
        error_log("srn_no: " . $srn_number . "\n", 3, "logs/cu_error_log.txt");
        
                error_log("sup1: " . $supplier . "\n", 3, "sup.log");
        ...
        $columns = [
            ...
            "o_invoice"             => $o_invoice,
            "deli_challan"          => $deli_challan,
            "test_cert"             => $test_cert,
            "om_manual"             => $om_manual,
            "amd_no"                => $amd_no,
            "cost_center"           => $cost_center,
            "deli_challan_no"       => $deli_challan_no,
    ...
    case 'datatable':
        ...
        if ($result->status) {
            ...
            foreach ($res_array as $key => $value) {
                ...
                error_log("srn_check_status: " . print_r($srn_check_status, true) . "\n", 3, "logs/srn_check_status_log.txt");
                ...
                // if ($srn_check_status == 1 && $srn_approve_status != 1) {
...
break;
    ...
    break;
        ...
        case 'cost_center':
            ...
            $cost_center  = cost_center_project($project_id);
    ...
    case 'get_purchase_order_no':
        ...
        error_log("po_vid: " . $vendor_id . "\n", 3, "logs/po_number.txt");
        error_log("po_pid: " . $project_id . "\n", 3, "logs/po_number.txt");
...
function generatesrn($label, &$labelData) {
    ...
    do {
        ...
        $srn = "SRN/$year/$label/$paddedNumber";
        $number++;
...
function get_po_number1($unique_id = '', $project_id = '', $company_id = '', $vendor_id = "")
{
    global $pdo;
    ...
    // Use array syntax consistently
    $where = [];
    $where[] = "(is_delete = 0 AND (
                  (gross_amount < 300000 AND status = 1)
               OR (gross_amount >= 300000 AND gross_amount < 1000000 AND lvl_2_status = 1)
               OR (gross_amount >= 1000000 AND lvl_3_status = 1)
              ))";
    ...
    error_log(print_r($states, true), 3, "logs/get_po.log");
