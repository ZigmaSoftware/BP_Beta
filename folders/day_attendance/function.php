...
//Staff count
function get_total_staff($entry_date, $work_location = null, $projects = []) {
    global $pdo;
    ...
    if (!empty($projects)) {
        if (is_array($projects)) {
            $project_list = array_map('trim', $projects);
            $where .= " AND work_location IN ('" . implode("','", $project_list) . "')";
        } else {
            $where .= " AND work_location = '" . trim($projects) . "'";
        }
    }

    $table_columns = ["COUNT(staff_id) as staff"];
    $table_details = [$table_name, $table_columns];

    $staff_count = $pdo->select($table_details, $where);

    return ($staff_count->status) ? ($staff_count->data[0]['staff'] ?? 0) : 0;
}


//Present count
function get_present_count($entry_date, $work_location = null, $projects = []) {
    global $pdo;

    $table_name = "view_bp_present";
    $table_columns = ["COUNT(*) as present_count"];
    ...
    if (!empty($projects)) {
        if (is_array($projects)) {
            $where .= " AND work_location IN ('" . implode("','", $projects) . "')";
        } else {
            $where .= " AND work_location = '" . trim($projects) . "'";
        }
    }
    ...
    return ($present_count->status) ? ($present_count->data[0]['present_count'] ?? 0) : 0;
}
...
//absent count
//absent count
function get_absent_count($entry_date, $projects = []) {
    global $pdo;

    $table_columns = ["COUNT(DISTINCT staff.unique_id) as absent_count"];
    $table_details = [
        "staff 
         LEFT JOIN view_staff_current_date_status vs 
         ON staff.unique_id = vs.staff_id 
         AND vs.entry_date = '".$entry_date."'",
        $table_columns
    ];

    $where  = "staff.is_active = 1 AND staff.is_delete = 0 
               AND vs.staff_id IS NULL";  // no attendance record = absent

    // Add project filter if given
    if (!empty($projects)) {
        if (is_array($projects)) {
            $safe_projects = array_map('trim', $projects);
            $where .= " AND staff.work_location IN ('" . implode("','", $safe_projects) . "')";
        } else {
            $where .= " AND staff.work_location = '" . trim($projects) . "'";
        }
    }
    ...
    if (!($absent_count->status)) {
        ...
        return $absent_count->data[0]['absent_count'] ?? 0;
    }
}
