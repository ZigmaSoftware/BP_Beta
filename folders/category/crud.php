...
switch ($action) {
    case 'createupdate':
        ...
        $code               = $_POST["code"];
        $final_code         = $sub_group_code.'-'.$code;
        ...
        $select_where       = 'group_unique_id =  "'.$group_unique_id.'" AND sub_group_unique_id =  "'.$sub_group_unique_id.'" AND (category_name = "'.$category_name.'" OR code = "'.$final_code.'") AND is_delete = 0  ';
    ...
    case 'datatable':
        ...
        $where = " is_delete = '0' ";
        if ($_POST['group_unique_id'] || $_POST['sub_group_unique_id']) {
            $where .= " AND (group_unique_id = '$_POST[group_unique_id]' OR sub_group_unique_id = '$_POST[sub_group_unique_id]')";
        }
        if ($_POST['group_unique_id'] && $_POST['sub_group_unique_id']) {
            $where .= " AND group_unique_id = '$_POST[group_unique_id]' AND sub_group_unique_id = '$_POST[sub_group_unique_id]'";
        }
        ...
        if ($result->status) {
            ...
            foreach ($res_array as $key => $value) {
                ...
                $value['category_name'] =  '<div>' .$value['category_name'] . '</div><div style="font-weight: bold;">' . $value['code'] .'</div>';
                $value['category_name'] = disname($value['category_name']);
 ...
 case 'toggle':
    ...
    case 'sub_group_name':
        ...
        $sub_group_name_options  = select_option($sub_group_name_options,"Select the Sub Group Name");
